...
def update_calculation():
    ...
    try:
        ...
        unit_price = 0
    total_revenue = consume_count * unit_price
    remaining_units_label.config(text=f"Remaining Units: {remaining_units}")
    total_revenue_label.config(text=f"Total Revenue: {consume_count} × {unit_price} = {total_revenue} TK")
...
style = ttk.Style()
style.configure("TButton", font=("Consolas", 14, "bold"), padding=10, relief="flat")
style.map("TButton", background=[("active", "#e0e0e0")])  # Hover effect

# Stylish entry box configuration
entry_style = {"font": ("Consolas", 14), "relief": "flat", "highlightthickness": 1, "highlightbackground": "#CCCCCC"}
...
producer_circle = tk.Label(main_frame, text="পিঠার কারিগর", bg="lightgray", width=15, height=8, font=("Helvetica", 12, "bold"))
producer_circle.grid(row=0, column=0, padx=20)

consumer_circle = tk.Label(main_frame, text="পিঠা খাদক", bg="lightgray", width=15, height=8, font=("Helvetica", 12, "bold"))
...
tk.Label(control_frame, text="Pitha Producer Speed:", font=("Consolas", 18, "bold")).grid(row=0, column=0, padx=5)
producer_speed_entry = tk.Entry(control_frame, **entry_style)
producer_speed_entry.insert(0, str(producer_speed))
producer_speed_entry.grid(row=0, column=1, padx=5)

tk.Label(control_frame, text="Pitha Consumer Speed:", font=("Consolas", 18, "bold")).grid(row=1, column=0, padx=5)
consumer_speed_entry = tk.Entry(control_frame, **entry_style)
consumer_speed_entry.insert(0, str(consumer_speed))
consumer_speed_entry.grid(row=1, column=1, padx=5)

tk.Label(control_frame, text="Pitha Storage:", font=("Consolas", 18, "bold")).grid(row=2, column=0, padx=5)
buffer_size_entry = tk.Entry(control_frame, **entry_style)
buffer_size_entry.insert(0, str(buffer_size))
buffer_size_entry.grid(row=2, column=1, padx=5)

apply_button = ttk.Button(control_frame, text="Apply", command=update_values, style="TButton")
apply_button.grid(row=3, column=0, columnspan=2, pady=5)

buttons_frame = tk.Frame(root)
buttons_frame.pack(pady=10)

start_button = ttk.Button(buttons_frame, text="Start", command=start_simulation, style="TButton")
start_button.grid(row=0, column=0, padx=10)

stop_button = ttk.Button(buttons_frame, text="Stop", command=stop_simulation, style="TButton")
stop_button.grid(row=0, column=1, padx=10)

reset_button = ttk.Button(buttons_frame, text="Reset", command=reset_simulation, style="TButton")
reset_button.grid(row=0, column=2, padx=10)

calculation_frame = tk.Frame(root)
calculation_frame.pack(pady=10)

tk.Label(calculation_frame, text="Per Unit Pitha Price:", font=("Consolas", 18, "bold")).grid(row=0, column=0, padx=5)
unit_price_entry = tk.Entry(calculation_frame, **entry_style)
unit_price_entry.insert(0, "0")
unit_price_entry.grid(row=0, column=1, padx=5)

remaining_units_label = tk.Label(calculation_frame, text="Remaining Pithas: 0", font=("Consolas", 18, "bold"))
remaining_units_label.grid(row=1, column=0, columnspan=2, pady=5)

total_revenue_label = tk.Label(calculation_frame, text="Total Revenue: 0 × 0 = 0 TK", font=("Consolas", 18, "bold"))
total_revenue_label.grid(row=2, column=0, columnspan=2, pady=5)

status_frame = tk.Frame(root)
status_frame.pack(pady=5)

produce_label = tk.Label(status_frame, text=f"Total Produced Pitha: {produce_count}", fg="green", font=("Consolas", 18, "bold"))
produce_label.grid(row=0, column=0, padx=20)

consume_label = tk.Label(status_frame, text=f"Total Consumed Pitha: {consume_count}", fg="blue", font=("Consolas", 18, "bold"))
...
# Adding footer lines with adjusted padding
footer_label1 = tk.Label(footer_frame, text="Developed by MD MOHIDUL ALAM | © 2024", bg="lightgray", font=("Consolas", 14, "italic"))
footer_label1.pack(pady=2)

footer_label2 = tk.Label(footer_frame, text="mohidul17@pstu.ac.bd", bg="lightgray", font=("Consolas", 12, "italic"))
footer_label2.pack(pady=2)
